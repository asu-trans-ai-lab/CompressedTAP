...
\newcommand{\docstamp}{Draft \docver\ \textbullet\ built \docdate\ \textbullet\ code
  ...
  rewritten, flow-weighted basis added; body in black, red = notes to co-authors}

% Author-facing notes (red). Remove this block and the \conote calls before submission.
\newcommand{\conote}[1]{{\color{red}\small\textsf{[Note to co-authors: #1]}}}
\newcommand{\conoteY}[1]{{\color{red}\small\textsf{[Note to Yuchao: #1]}}}
...
The weighted basis is better on both axes at every point, and the accuracy gain exceeds $50\%$
wherever $\bar K \ge 8.64$. The trend is the explanation: a rich candidate pool is rich
precisely in paths that carry almost no flow, and the unweighted criterion spends its rank
budget representing them. This is why the regime that was least favourable in
Section~\ref{sec:road-networks} --- rich pools, where the compressed problem showed both the
largest gap and the smallest time advantage --- is the regime the weighting repairs most.

The gain is regime-dependent, and the grid family shows the other side. Across its twenty
cells the weighted basis lowers the mean feasible gap from $1.285\%$ to $1.112\%$ while
lowering the mean speedup slightly, from $2.58\times$ to $2.47\times$. At high richness it wins
outright --- at $N=32$, $K=64$ it reaches $5.41\times$ at a gap of $0.000\%$ against
$3.87\times$ at $0.406\%$ --- whereas at moderate $K$ it converts part of the time advantage
into accuracy. The same happens in the extreme-compression case: on Sioux Falls at $91\%$
variable reduction the gap falls by $11$ to $20\%$ relative, at two to three times the solution
time. The conclusion to draw is therefore not that weighting is uniformly faster, but that it
makes the subspace criterion consistent with the objective: on the route-rich instances the
method targets it improves both time and accuracy, and under extreme compression it converts
speed into accuracy.
...
\subsection{The Speed--Accuracy Trade-off}
\label{sec:tradeoff}


The experiments above vary the threshold, the rank, the pool, the operator, the offset, the
network and the implementation. Because every configuration yields both a speedup and a
feasible objective gap, they can be placed on a single trade-off plane, which is the form in
which the question ``is the speedup worth the accuracy loss?'' is properly answered.
Figure~\ref{fig:tradeoff} plots all $71$ measured configurations.

\begin{figure}[h]
\centering
\includegraphics[width=0.78\textwidth]{fig/fig_tradeoff.pdf}
\caption{Speed--accuracy trade-off over all measured configurations. Each point is
one combination of instance, threshold, rank, implementation and offset; the horizontal dashed
line is parity with the uncompressed solve and the shaded band marks an accuracy price below
$0.1\%$. Open markers are the offset-free variant, which is dominated on both axes.}
\label{fig:tradeoff}
\end{figure}

Three features matter. First, the trade-off is \emph{not} a frontier along which speed must be
purchased with accuracy: thirteen configurations attain a speedup above one at a feasible
objective gap of at most $0.1\%$, the best of them $6.82\times$ at a gap of $0.0000\%$. In
that region compression is close to free, and the relevant design question is whether an
instance admits such a configuration, not how much accuracy to trade. Second, the largest
speedup observed, $10.31\times$, does sit at a gap of $3.44\%$, so the extreme of the range
is genuinely a trade; the intermediate region is where the method is useful. Third, the
offset-free points lie below and to the right of the rest --- worse on both axes
simultaneously --- which is the trade-off plane's version of the conclusion of
\S\ref{sec:offset-study}: removing the anchor does not buy speed at the cost of accuracy, it
loses both.
