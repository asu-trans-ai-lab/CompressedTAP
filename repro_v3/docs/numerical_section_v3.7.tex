...
Section~\ref{sec:ceiling} establishes a structural ceiling on how much of the problem can be
compressed at all --- route richness alone caps the reduction at $1-1/\bar K$ --- and uses it
to say in advance which instances the method can help.
...
\subsection{A Structural Ceiling on Compression, and What It Decides}
\label{sec:ceiling}

Sections~\ref{sec:cost-model}--\ref{sec:repeated} treat the threshold and the rank as free
parameters. They are not entirely free, and the binding constraint is simple enough to state
exactly. Every OD pair must retain at least one explicit path, so the number of majors cannot
fall below $\ell$ and the variable reduction obeys
\begin{equation}
\label{eq:ceiling}
\text{reduction}\;=\;\frac{n-s-r}{n}\;\le\;\frac{n-\ell-r}{n}\;\approx\;1-\frac{1}{\bar K},
\qquad \bar K=\frac{n}{\ell}.
\end{equation}
The achievable compression is therefore fixed by route richness alone, before any tuning.

Table~\ref{tab:ceiling} shows every instance in this study sitting essentially at that
ceiling. Raising the threshold on Chicago Regional from $\tau=1.03$ through $3$, $8$ and $20$
moves the reduction only from $84.0\%$ to $86.3\%$, $86.6\%$ and $86.6\%$: at $\tau=20$ the
split retains $270{,}716$ majors against $270{,}598$ OD pairs, i.e.\ almost exactly one path
per OD, and no larger threshold can do better.

\begin{table}[h]
\centering
\caption{The reduction ceiling. $\bar K$ alone determines how much of the problem can be
compressed; the measured reduction sits at the ceiling on every instance except the sparsest,
where the threshold rather than the ceiling binds.}
\label{tab:ceiling}
{\small
\begin{tabular}{lrrrrr}
\toprule
instance & $n$ & $\ell$ & $\bar K$ & ceiling $1-1/\bar K$ & measured\\
\midrule
Chicago Sketch (submitted) & 42{,}774    & 17{,}464 & 2.45  & 59.2\% & 53.2\%\\
Chicago Sketch (E0)        & 229{,}175   & 65{,}728 & 3.49  & 71.3\% & 70.3\%\\
Chicago Regional (E0)      & 2{,}024{,}525 & 270{,}598 & 7.48 & \textbf{86.6\%} & \textbf{86.6\%}\\
Chicago Sketch (K10)       & 903{,}516   & 81{,}435 & 11.09 & 91.0\% & 89.8\%\\
Chicago Sketch (K15)       & 1{,}323{,}999 & 86{,}304 & 15.34 & 93.5\% & 92.4\%\\
\bottomrule
\end{tabular}}
\end{table}

Combining \eqref{eq:ceiling} with the measurements gives the study its sharpest statement.
In the compiled implementation the compressed solve beat the uncompressed one only where the
reduction reached about $90\%$; the two Chicago Sketch pools that did so returned
$3.06\times$ and $3.26\times$ at feasible objective gaps of at most $0.084\%$
(Table~\ref{tab:cpp-closure}). Chicago Regional cannot reach that reduction at all: its pool
carries $\bar K=7.48$, so \eqref{eq:ceiling} caps it at $86.6\%$, and indeed the compressed
solve there was slower than the uncompressed one for every rank tried. Inverting the
requirement, a network needs
\[
1-\frac{1}{\bar K}\;\ge\;0.90 \qquad\Longleftrightarrow\qquad \bar K\;\gtrsim\;10
\]
before the compiled compressed solve can be expected to win. This is a property of the
candidate pool, not of the network, and it is checkable in advance from two integers.

\begin{table}[h]
\centering
\caption{C\texttt{++} closure runs on Chicago Sketch, weighted basis, single thread. Each
baseline is the uncompressed solve of the same instance in the same session.}
\label{tab:cpp-closure}
{\small
\begin{tabular}{lrrrrrr}
\toprule
instance & $\bar K$ & reduction & baseline (s) & $r$ & CPU (s) & speedup\\
\midrule
Chicago Sketch (K10) & 11.09 & 89.8\% & 301.2 & 6  & 98.5  & \textbf{3.06}\\
                     &       &        &       & 10 & 122.6 & 2.46\\
                     &       &        &       & 20 & 183.4 & 1.64\\
\midrule
Chicago Sketch (K15) & 15.34 & 92.4\% & 583.5 & 6  & 178.9 & \textbf{3.26}\\
                     &       &        &       & 10 & 201.7 & 2.89\\
                     &       &        &       & 20 & 294.4 & 1.98\\
\bottomrule
\end{tabular}}
...
Two remarks close the argument. The speedup falls monotonically with the rank in both pools,
as \eqref{eq:cost-rule} requires, and the best rank is the smallest tried, $r=6$, consistent
with the rule $2r\lesssim$ links-per-path ($17$ here). And the earlier conclusion that
compression did not pay in the compiled implementation on Chicago Sketch was an artefact of
running $r=50$ on a network with $17$ links per path --- six times the rank the rule allows.
