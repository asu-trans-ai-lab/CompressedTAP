...
(a) plain to flow-weighted subspace, (b) affine offset present to removed. The horizontal axis
is exact; the vertical axis carries the measurement noise of Section~\ref{sec:setup}, drawn as
the error bar, so vertical displacements shorter than it are not effects.}
\label{fig:paired}
\end{figure}

Two readings follow, and the noise floor decides which is which. Horizontally, the arrows in
panel~(a) point left in $27$ of $34$ pairs: the weighted subspace is an accuracy improvement,
and it is measured exactly. Vertically they scatter, and only $2$ of $34$ exceed the noise
floor, so the apparent speed gains are not resolvable and we do not claim them. Panel~(b) is
the offset ablation on the weighted representation, and the gap rises in $16$ of $20$ grid
cells, confirming that removing the anchor loses accuracy whichever subspace is used.
