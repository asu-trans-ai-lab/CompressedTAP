...
\subsection{Frank--Wolfe Retains Its Convex-Combination Interpretation}
\label{sec:fw-interpretation}

Because the feasible set of \eqref{eq:compressed-fw} is a product of OD
simplices, the FW linearization step
\begin{equation*}
...
$z^{k+1} = (1 - \alpha_k) z^k + \alpha_k w^k$ with $\alpha_k \in [0,1]$ stays
in $\mathcal Z$ by convexity, and in original path-flow coordinates
\begin{equation*}
x^{k+1} = G z^{k+1} = (1 - \alpha_k)\, x^k + \alpha_k\, G w^k ,
\end{equation*}
so every iterate remains a convex combination of feasible path-flow
assignments of the original problem. The representation does not introduce a
...
gaps, and residuals are deterministic.} tests this by adding exactly one
component --- a second update operator --- and changing nothing else: a
\emph{latent projected gradient} (LPG) method that shares with latent FW the
atom set, the latent gradient $\bar B\, t(\bar B' z + v_0)$, the OD demand
simplices, the objective, and even the stopping certificate (the identical FW
duality gap, computed from the same linear minimization oracle). The update is
a Euclidean simplex projection with a safeguarded Barzilai--Borwein trial step
and Armijo backtracking on the projected direction. The protocol enforces
...
\begin{tabular}{rrrrr}
\toprule
$K$ & PG speedup & FW speedup & RC gap (\%) & C gap (\%) \\
\midrule
...
of $95\times$ (FW) and $4.3\times$ (PG) on that instance. On the
...
path pool, and LPG on the same interface atoms, and requires agreement of
their optima. On the spread-demand Chicago instance, LPG matches the SLSQP
...
reduced gradient $=$ FW $=$ LPG, all through the standard problem-layer
